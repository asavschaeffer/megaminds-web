...
export const PricingCalculator = ({ apiData, chatData, pricingSources }: PricingCalculatorProps) => {
  ...
  const baseCost = useMemo(
    () => (apiData.baseModel.input + apiData.baseModel.output) * tokens,
    [apiData.baseModel.input, apiData.baseModel.output, tokens],
  )
  ...
  }, [apiData.baseModel, apiData.competitors, tokens])
  ...
  const apiTabId = `pricing-tab-api-${instanceId}`
  const chatTabId = `pricing-tab-chat-${instanceId}`
  const apiPanelId = `pricing-panel-api-${instanceId}`
  const chatPanelId = `pricing-panel-chat-${instanceId}`
            ...
            <div id={apiPanelId} role="tabpanel" aria-labelledby={apiTabId} aria-live="polite">
              ...
              <table className="w-full">
                ...
                <tbody className="space-y-2">
                  {sortedModels.map((model) => {
                    ...
                    const diffPercent = cost === 0 ? 0 : ((cost - baseCost) / cost) * 100
                    ...
                    return (
                      <tr
                        key={`${model.name}-${isBaseModel ? 'base' : 'comp'}`}
                        className={`flex items-center justify-between p-3 border-b border-neutral-100 dark:border-neutral-800 last:border-0 ${isBaseModel
                      ...
                      >
                        ...
                        <td className="text-left">
                          <output
                            className={`tabular-nums ${isBaseModel
              ...
              {pricingSources && pricingSources.length > 0 ? (
                <div className="mt-4 text-xs text-neutral-500 dark:text-neutral-400">
                  <p className="font-semibold text-neutral-600 dark:text-neutral-300">Sources of truth</p>
                  <ul className="mt-1 space-y-1 list-none">
                    {pricingSources.map((source, idx) => (
                      <li key={`${source.href}-${idx}`} className="flex items-center gap-2">
                        <a href={source.href} className="underline underline-offset-4">
                          {source.label}
                        </a>
                        {source.provider && <span className="text-neutral-400">({source.provider})</span>}
                      </li>
                    ))}
                  </ul>
                </div>
