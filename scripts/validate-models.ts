 ...
 *   5. Malformed MDX `## ` headings the section parser silently drops.
 *
 * (Analysis-item length is no longer audited here — the AnalysisItemSchema
 * `claim` cap enforces it at registry load, so an over-length item can never
 * reach this script.)
 *
 * Flags:
